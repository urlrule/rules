#!/usr/bin/perl -w
#http://photo.shawei.com/album/girls/girlstopics/18201.html
#Thu Oct 14 02:17:17 2010
use strict;
...
sub _process {
    ...
    $prefix =~ s/\.html$//;
    foreach(@html) {
        if((!$total) and /var\s*total\s*=\s*(\d+)/) {
            $total = $1 ;
            last;
        }
    }
    for(my $i=0;$i<$total;$i=$i+6) {
        push @pass_data,"$prefix/$i.html";
    }
    return (
        count=>scalar(@data),
        data=>[@data],
